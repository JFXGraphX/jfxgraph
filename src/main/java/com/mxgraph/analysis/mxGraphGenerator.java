...
public class mxGraphGenerator {
	...
	public void getNullGraph(mxAnalysisGraph aGraph, int numVertices) {
		...
		for (int i = 0; i < numVertices; i++) {
			graph.insertVertex(parent, null, new Integer(i).toString(), i * 50,
					0, 25, 25);
		}
	};
	...
	public void getCompleteGraph(mxAnalysisGraph aGraph, int numVertices) {
		...
		for (int i = 0; i < numVertices; i++) {
			vertices[i] = graph.insertVertex(parent, null,
					new Integer(i).toString(), i * 50, 0, 25, 25);
		}
		...
		for (int i = 0; i < numVertices; i++) {
			...
			for (int j = 0; j < numVertices; j++) {
				...
				if (vertex1 != vertex2
						&& !mxGraphStructure.areConnected(aGraph, vertex1,
								vertex2)) {
					graph.insertEdge(parent, null, getNewEdgeValue(aGraph),
							vertex1, vertex2);
				}
			}
		}
		...
		for (int j = 0; j < numRows; j++) {
			for (int i = 0; i < numColumns; i++) {
				...
				if (i > 0) {
					...
					graph.insertEdge(parent, null, getNewEdgeValue(aGraph),
							vertices[vertexCount - 1], currVertex);
				}

				if (j > 0) {
					// connect with previous y
					graph.insertEdge(parent, null, getNewEdgeValue(aGraph),
							vertices[vertexCount - numColumns], currVertex);
				}
	 ...
	 *            - number of columns in the grid graph
	 */
	public void setGridGraphSpacing(mxAnalysisGraph aGraph, double xSpacing,
			double ySpacing, int numColumns, int numRows) {
		mxGraph graph = aGraph.getGraph();
	...
	public void getBipartiteGraph(mxAnalysisGraph aGraph,
			int numVerticesGroup1, int numVerticesGroup2) {
		...
		for (int i = 0; i < numVerticesGroup1; i++) {
			...
			Object destVertex = vertices[getRandomInt(numVerticesGroup1,
					numVertices - 1)];
			graph.insertEdge(parent, null, getNewEdgeValue(aGraph), currVertex,
					destVertex);
		}

		for (int j = 0; j < numVerticesGroup2; j++) {
			Object currVertex = vertices[numVerticesGroup1 + j];
			int edgeNum = aGraph.getOpposites(
					aGraph.getEdges(currVertex, null, true, true, false, true),
					currVertex, true, true).length;

			if (edgeNum == 0) {
				Object destVertex = vertices[getRandomInt(0,
						numVerticesGroup1 - 1)];
				graph.insertEdge(parent, null, getNewEdgeValue(aGraph),
	...
	public void setBipartiteGraphSpacing(mxAnalysisGraph aGraph,
			int numVerticesGroup1, int numVerticesGroup2, double vertexSpacing,
			double groupSpacing) {
		if (numVerticesGroup1 < 0 || numVerticesGroup2 < 0) {
			throw new IllegalArgumentException();
		}
		...
		for (int i = numVerticesGroup1; i < numVerticesGroup1
				+ numVerticesGroup2; i++) {
			Object currVertex = vertices[i];
			mxGeometry geometry = model.getGeometry(currVertex);
			geometry.setX(groupSpacing);
			geometry.setY(group2StartY + (i - numVerticesGroup1)
					* vertexSpacing);
		}
	};
	...
	public void getKnightGraph(mxAnalysisGraph aGraph, int xDim, int yDim) {
		...
		for (int i = 0; i < (xDim * yDim); i++) {
			...
			Object[] neighborMoves = getKnightMoveVertexes(aGraph, xDim, yDim,
			...
			for (int j = 0; j < neighborMoves.length; j++) {
				...
				if (!mxGraphStructure.areConnected(aGraph, vertices[i],
						neighborMoves[j])) {
					graph.insertEdge(parent, null, getNewEdgeValue(aGraph),
							vertices[i], neighborMoves[j]);
				}
	 ...
	 */
	public Object[] getKnightMoveVertexes(mxAnalysisGraph aGraph, int xDim,
			int yDim, int xCoord, int yCoord) {
		if (xCoord > xDim || yCoord > yDim || xDim < 1 || yDim < 1
				|| xCoord < 1 || yCoord < 1) {
			throw new IllegalArgumentException();
		}
	 ...
	 */
	private Object getVertexFromGrid(Object[] vertices, int xDim, int yDim,
	...
	public void getKingGraph(mxAnalysisGraph aGraph, int xDim, int yDim) {
		...
		for (int i = 0; i < (xDim * yDim); i++) {
			currCoords = getVertexGridCoords(xDim, yDim, i);
			Object[] neighborMoves = getKingMoveVertexes(aGraph, xDim, yDim,
					currCoords[0], currCoords[1]);
	 ...
	 */
	public Object[] getKingMoveVertexes(mxAnalysisGraph aGraph, int xDim,
			int yDim, int xCoord, int yCoord) {
	...
	public void getPetersenGraph(mxAnalysisGraph aGraph) {
		...
		graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[0],
				vertices[2]);
		graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[0],
				vertices[8]);
		graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[0],
				vertices[9]);
		graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[1],
				vertices[2]);
		graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[1],
				vertices[5]);
		graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[1],
				vertices[7]);
		graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[2],
				vertices[4]);
		graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[3],
				vertices[4]);
		graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[3],
				vertices[7]);
		graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[3],
				vertices[9]);
		graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[4],
				vertices[6]);
		graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[5],
				vertices[6]);
		graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[5],
				vertices[9]);
		graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[6],
				vertices[8]);
		graph.insertEdge(parent, null, getNewEdgeValue(aGraph), vertices[7],
				vertices[8]);
	};
	...
	public void getStarGraph(mxAnalysisGraph aGraph, int numVertices) {
		...
		for (int i = 0; i < numVertexesInPerimeter; i++) {
			graph.insertEdge(parent, null, getNewEdgeValue(aGraph),
					vertices[numVertexesInPerimeter], vertices[i]);
		}
	};
	...
	public void setStarGraphLayout(mxAnalysisGraph aGraph, double graphSize) {
		...
		for (int i = 0; i < numVertexesInPerimeter; i++) {
			...
			x = Math.round(centerX
					+ Math.round(graphSize * Math.sin(currRatio) / 2));
			y = Math.round(centerY
					- Math.round(graphSize * Math.cos(currRatio) / 2));
			Object currVertex = vertices[i];
			mxGeometry geometry = model.getGeometry(currVertex);
			geometry.setX(x);
			geometry.setY(y);
	...
	public void getWheelGraph(mxAnalysisGraph aGraph, int numVertices) {
		...
		for (int i = 0; i < numVerticesInPerimeter; i++) {
			graph.insertEdge(parent, null, getNewEdgeValue(aGraph),
					vertices[numVerticesInPerimeter], vertices[i]);
	...
	public void getFriendshipWindmillGraph(mxAnalysisGraph aGraph,
			int numBranches, int branchSize) {
		...
		for (int i = 0; i < numBranches; i++) {
			for (int j = 0; j < branchSize; j++) {
				vertices[vertexCount] = graph.insertVertex(parent, null,
						new Integer(vertexCount).toString(), 0, 0, 25, 25);
				vertexCount++;
			}
		}

		vertices[numVertices - 1] = graph.insertVertex(parent, null,
				new Integer(numVertices - 1).toString(), 0, 0, 25, 25);
		...
		for (int i = 0; i < numBranches; i++) {
			...
			graph.insertEdge(parent, null, getNewEdgeValue(aGraph), oldVertex,
					currVertex);
		}
	};
	 ...
	 *            a windmill graph
	 */
	public void getWindmillGraph(mxAnalysisGraph aGraph, int numBranches,
			int branchSize) {
		if (numBranches < 2 || branchSize < 2) {
			throw new IllegalArgumentException();
		}
		...
		for (int i = 0; i < numBranches; i++) {
			for (int j = 0; j < branchSize; j++) {
				...
				if (!mxGraphStructure.areConnected(aGraph, centerVertex,
						vertex1)) {
					graph.insertEdge(parent, null, getNewEdgeValue(aGraph),
							centerVertex, vertex1);
				}
	 ...
	 * @param graphSize
	 */
	public void setWindmillGraphLayout(mxAnalysisGraph aGraph, int numBranches,
			int numVerticesInBranch, double graphSize) {
		...
		for (int i = 0; i < numBranches; i++) {
			for (int j = 0; j < numVerticesInBranch; j++) {
				double currSize = this.getRingSize(j + 1, numVerticesInBranch,
						graphSize);
				...
				if (isBranchSizeEven && j == middleIndex - 1) {
					// full size
					currRatio = ((double) i - (0.0005f * graphSize / Math.pow(
							numVerticesInBranch, 1)))
					...
					if (j + 1 < middleIndex) {
						// before middle
						currRatio = ((double) (i - 1f
								/ Math.pow(currSize, 0.25) + 0.00000000000015f * Math
								.pow(currSize, 4)) / (double) numVertexesInPerimeter);
					} else {
						// after middle
						currRatio = ((double) (i + 1f
								/ Math.pow(currSize, 0.25) - 0.00000000000015f * Math
				...
				currRatio = currRatio * (double) Math.PI;
				x = Math.round(centerX
						+ Math.round(currSize * Math.sin(currRatio) / 2));
				y = Math.round(centerY
						- Math.round(currSize * Math.cos(currRatio) / 2));
				// shoot
	...
	private double getRingSize(int currIndex, int branchSize, double fullSize) {
		if (currIndex < 1 || currIndex > branchSize || branchSize < 1
				|| fullSize < 0) {
			throw new IllegalArgumentException();
		}
		...
		if (currIndex == middleIndex
				|| (isBranchSizeEven && currIndex == middleIndex + 1)) {
			// full size
			return fullSize;
		...
		return (((float) Math.pow(currIndex, 0.75) / (float) Math.pow(
				middleIndex, 0.75)) * fullSize);
	};
	 ...
	 */
	public void getSimpleRandomGraph(mxAnalysisGraph aGraph, int numNodes,
			int numEdges, boolean allowSelfLoops, boolean allowMultipleEdges,
		...
		for (int i = 0; i < numEdges; i++) {
			...
			do {
				goodPair = true;
				startVertex = vertices[(int) Math.round(Math.random()
						* (vertices.length - 1))];
				endVertex = vertices[(int) Math.round(Math.random()
						* (vertices.length - 1))];

				if (!allowSelfLoops && startVertex.equals(endVertex)) {
					goodPair = false;
				} else if (!allowMultipleEdges
						&& mxGraphStructure.areConnected(aGraph, startVertex,
								endVertex)) {
					goodPair = false;
				}
			} while (!goodPair);
	...
	public void getSimpleRandomTree(mxAnalysisGraph aGraph, int vertexCount) {
		...
		this.getSimpleRandomGraph(aGraph, vertexCount, edgeCount, false, false,
				true);
		...
		try {
			mxGraphStructure.makeTreeDirected(
					aGraph,
					vertices[(int) Math.round(Math.random()
							* (vertices.length - 1))]);
		} catch (StructuralException e) {
			System.out.println(e);
		}
	...
	public Double getNewEdgeValue(mxAnalysisGraph aGraph) {
		if (getGeneratorFunction() != null) {
			mxGraph graph = aGraph.getGraph();
			return getGeneratorFunction().getCost(
					graph.getView().getState(graph.getDefaultParent()));
		} else {
			return null;
		}
	};
	 ...
	 *             edges)
	 */
	public void oneSpanningTree(mxAnalysisGraph aGraph, boolean forceConnected,
			boolean forceSimple) throws StructuralException {
		mxGraph graph = aGraph.getGraph();
	 ...
	 */
	public void getKnightTour(mxAnalysisGraph aGraph, int xDim, int yDim,
			int startVertexValue) throws StructuralException {
		if (xDim < 5 || yDim < 5) {
			throw new IllegalArgumentException();
		}
		...
		Object nextMove = getNextKnightMove(aGraph, xDim, yDim, currCoords[0],
				currCoords[1], resultPath);
		...
		while (nextMove != null) {
			...
			currValue = (int) costFunction.getCost(new mxCellState(view,
					nextMove, null));
			currCoords = getVertexGridCoords(xDim, yDim, currValue);
			oldMove = nextMove;
			nextMove = getNextKnightMove(aGraph, xDim, yDim, currCoords[0],
					currCoords[1], resultPath);
		}
		...
		if (resultPath.size() < vertexNum) {
			...
			throw new StructuralException(
					"Could not generate a correct Knight tour with size "
							+ xDim + " x " + yDim + ".");
		}
	};
	 ...
	 * @return
	 */
	private Object getNextKnightMove(mxAnalysisGraph aGraph, int xDim,
			int yDim, int xCoord, int yCoord, ArrayList<Object> resultPath) {
		Object[] possibleMoves = getKnightMoveVertexes(aGraph, xDim, yDim,
				xCoord, yCoord);
		...
		for (int i = 0; i < possibleMoves.length; i++) {
			int currValue = (int) costFunction.getCost(new mxCellState(view,
					possibleMoves[i], null));
			int[] currCoords = getVertexGridCoords(xDim, yDim, currValue);
			int currMoveNum = getPossibleKnightMoveCount(aGraph, xDim, yDim,
					currCoords[0], currCoords[1]);
	 ...
	 * @return
	 */
	private int getPossibleKnightMoveCount(mxAnalysisGraph aGraph, int xDim,
			int yDim, int xCoord, int yCoord) {
		// check all possible 8 locations
		...
		if (currX > 0 && currX <= xDim && currY > 0 && currY <= yDim) {
			if (aGraph.getEdges(
					getVertexFromGrid(vertices, xDim, yDim, currX, currY),
					parent, false, true).length == 0) {
				possibleMoveCount++;
			}
		}
