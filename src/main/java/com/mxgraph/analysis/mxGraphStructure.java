...
public class mxGraphStructure {
	...
	public static boolean isCyclicUndirected(mxAnalysisGraph aGraph) {
		...
		Object[] cells = model.cloneCells(
				aGraph.getChildCells(graph.getDefaultParent(), true, true),
				true);
	...
	private static Object getUndirectedLeaf(mxAnalysisGraph aGraph) {
		...
		for (int i = 0; i < vertexNum; i++) {
			...
			int edgeCount = aGraph.getEdges(currVertex, parent, true, true,
					false, true).length;
	...
	public static boolean isSimple(mxAnalysisGraph aGraph) {
		...
		for (int i = 0; i < edges.length; i++) {
			...
			if (aGraph.getTerminal(currEdge, true) == aGraph.getTerminal(
					currEdge, false)) {
				return false;
			}
			...
			for (int j = 0; j < edges.length; j++) {
				...
				if (currEdge != currEdge2) {
					if (aGraph.getTerminal(currEdge, true) == aGraph
							.getTerminal(currEdge2, true)
							&& aGraph.getTerminal(currEdge, false) == aGraph
									.getTerminal(currEdge2, false)) {
						return false;
					}
	...
	public static boolean isTree(mxAnalysisGraph aGraph) {
		if (isConnected(aGraph) && !isCyclicUndirected(aGraph)
				&& isSimple(aGraph)) {
			return true;
		}
	 ...
	 *         are more
	 */
	static public Object getLowestDegreeVertex(mxAnalysisGraph aGraph,
			Object[] omitVertex) {
		Object[] vertices = aGraph.getChildVertices(aGraph.getGraph()
				.getDefaultParent());
		...
		for (int i = 0; i < vertexCount; i++) {
			if (omitVertex == null || !omitList.contains(vertices[i])) {
				int currEdgeCount = aGraph.getEdges(vertices[i], null, true,
						true, true, true).length;
	...
	public static boolean areConnected(mxAnalysisGraph aGraph,
		...
		Object currEdges[] = aGraph.getEdges(sourceVertex, aGraph.getGraph()
				.getDefaultParent(), true, true, false, true);
		List<Object> neighborList = Arrays.asList(aGraph.getOpposites(
				currEdges, sourceVertex, true, true));
	...
	public static void makeConnected(mxAnalysisGraph aGraph) {
		...
		for (int i = 1; i < componentNum; i++) {
			Object sourceVertex = components[i - 1][(int) Math.round(Math
					.random() * (components[i - 1].length - 1))];
			Object targetVertex = components[i][(int) Math.round(Math.random()
					* (components[i].length - 1))];
			graph.insertEdge(parent, null, aGraph.getGenerator()
					.getNewEdgeValue(aGraph), sourceVertex, targetVertex);
		}
	};
	...
	public static Object[][] getGraphComponents(mxAnalysisGraph aGraph) {
		...
		ArrayList<Object> unvisitedVertexList = new ArrayList<Object>(
				Arrays.asList(vertices));
		boolean oldDirectedness = mxGraphProperties.isDirected(
				aGraph.getProperties(), mxGraphProperties.DEFAULT_DIRECTED);
	...
	public static void makeTreeDirected(mxAnalysisGraph aGraph,
			Object startVertex) throws StructuralException {
		if (isTree(aGraph)) {
			mxGraphProperties.setDirected(aGraph.getProperties(), false);
			...
			for (int i = 0; i < bFSList.size(); i++) {
				...
				Object currEdges[] = aGraph.getEdges(parentVertex, parent,
						true, true, false, true);
				Object[] neighbors = aGraph.getOpposites(currEdges,
						parentVertex, true, true);
				...
				for (int j = 0; j < neighbors.length; j++) {
					...
					if (childIndex > i) {
						...
						Object currEdge = getConnectingEdge(aGraph,
								parentVertex, currVertex);
	...
	public static Object getConnectingEdge(mxAnalysisGraph aGraph,
			Object vertexOne, Object vertexTwo) {
		mxIGraphModel model = aGraph.getGraph().getModel();
		Object[] edges = aGraph.getEdges(vertexOne, null, true, true, false,
				true);
	...
	public static Object getDirectedLeaf(mxAnalysisGraph aGraph, Object parent) {
		...
		for (int i = 0; i < vertexNum; i++) {
			currVertex = vertices[i];
			int inEdgeCount = aGraph.getEdges(currVertex, parent, true, false,
					false, true).length;
			int outEdgeCount = aGraph.getEdges(currVertex, parent, false, true,
					false, true).length;
	...
	public static void complementaryGraph(mxAnalysisGraph aGraph) {
		...
		for (int i = 0; i < vertexCount; i++) {
			...
			for (int j = 0; j < vertexCount; j++) {
				...
				if (shouldConnect) {
					graph.insertEdge(parent, null, null, currVertex,
							targetVertex);
				}
			}
	...
	public static Object getVertexWithValue(mxAnalysisGraph aGraph, int value) {
		...
		for (int i = 0; i < childNum; i++) {
			...
			vertexValue = (int) costFunction.getCost(new mxCellState(view,
					currVertex, null));
	...
	public static void setDefaultGraphStyle(mxAnalysisGraph aGraph,
		...
		if (resetEdgeValues) {
			...
			for (int i = 0; i < edges.length; i++) {
				model.setValue(edges[i],
						aGraph.getGenerator().getNewEdgeValue(aGraph));
			}
		}
	};
	...
	public static int indegree(mxAnalysisGraph aGraph, Object vertex) {
		...
		if (mxGraphProperties.isDirected(aGraph.getProperties(),
				mxGraphProperties.DEFAULT_DIRECTED)) {
			return aGraph.getEdges(vertex,
					aGraph.getGraph().getDefaultParent(), true, false, true,
					true).length;
		} else {
			return aGraph.getEdges(vertex,
					aGraph.getGraph().getDefaultParent(), true, true, true,
					true).length;
		}
	};
	...
	public static int outdegree(mxAnalysisGraph aGraph, Object vertex) {
		if (mxGraphProperties.isDirected(aGraph.getProperties(),
				mxGraphProperties.DEFAULT_DIRECTED)) {
			return aGraph.getEdges(vertex,
					aGraph.getGraph().getDefaultParent(), false, true, true,
					true).length;
		} else {
			return aGraph.getEdges(vertex,
					aGraph.getGraph().getDefaultParent(), true, true, true,
	...
	public static boolean isCutVertex(mxAnalysisGraph aGraph, Object vertex) {
		...
		if (aGraph.getEdges(vertex, null, true, true, false, true).length >= 2) {
			...
			Object newVertex = getVertexWithValue(
					aGraphCopy,
					(int) aGraph
							.getGenerator()
							.getCostFunction()
							.getCost(
									new mxCellState(graph.getView(), vertex,
											null)));
	...
	public static Object[] getCutVertices(mxAnalysisGraph aGraph) {
		...
		Object[] vertexes = aGraph.getChildVertices(aGraph.getGraph()
				.getDefaultParent());
	...
	public static boolean isCutEdge(mxAnalysisGraph aGraph, Object edge) {
		...
		int srcValue = (int) costFunction.getCost(new mxCellState(view, aGraph
				.getTerminal(edge, true), null));
		int destValue = (int) costFunction.getCost(new mxCellState(view, aGraph
				.getTerminal(edge, false), null));

		if (aGraph.getTerminal(edge, false) != null
				|| aGraph.getTerminal(edge, true) != null) {
			Object[] cells = model.cloneCells(
					aGraph.getChildCells(graph.getDefaultParent(), true, true),
					true);
			mxGraphModel modelCopy = new mxGraphModel();
			...
			Object[] edges = aGraphCopy.getChildEdges(aGraphCopy.getGraph()
					.getDefaultParent());
			Object currEdge = edges[0];
			mxCostFunction costFunctionCopy = aGraphCopy.getGenerator()
					.getCostFunction();
			mxGraphView viewCopy = graphCopy.getView();

			int currSrcValue = (int) costFunctionCopy.getCost(new mxCellState(
					viewCopy, aGraphCopy.getTerminal(currEdge, true), null));
			int currDestValue = (int) costFunctionCopy.getCost(new mxCellState(
					viewCopy, aGraphCopy.getTerminal(currEdge, false), null));
			int i = 0;

			while (currSrcValue != srcValue || currDestValue != destValue) {
				i++;
				currEdge = edges[i];
				currSrcValue = Integer.parseInt((String) modelCopy
						.getValue(aGraphCopy.getTerminal(currEdge, true)));
				currDestValue = Integer.parseInt((String) modelCopy
						.getValue(aGraphCopy.getTerminal(currEdge, false)));
			}
	...
	public static Object[] getCutEdges(mxAnalysisGraph aGraph) {
		...
		Object[] edges = aGraph.getChildEdges(aGraph.getGraph()
				.getDefaultParent());
	...
	public static Object[] getSourceVertices(mxAnalysisGraph aGraph)
		...
		if (!mxGraphProperties.isDirected(aGraph.getProperties(),
			...
			throw new StructuralException(
					"The graph is undirected, so it can't have source vertices.");
		}
		...
		for (int i = 0; i < vertices.length; i++) {
			Object currVertex = vertices[i];
			Object[] outEdges = aGraph.getEdges(vertices[i], null, false, true,
					true, true);
			Object[] inEdges = aGraph.getEdges(vertices[i], null, true, false,
					true, true);
	...
	public static Object[] getSinkVertices(mxAnalysisGraph aGraph)
		...
		if (!mxGraphProperties.isDirected(aGraph.getProperties(),
				mxGraphProperties.DEFAULT_DIRECTED)) {
			throw new StructuralException(
					"The graph is undirected, so it can't have sink vertices.");
		}
	...
	public static boolean isBiconnected(mxAnalysisGraph aGraph) {
		int edgeCount = aGraph.getChildEdges(aGraph.getGraph()
