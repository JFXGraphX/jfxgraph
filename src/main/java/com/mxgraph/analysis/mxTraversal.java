...
public class mxTraversal {
	 ...
	 * @param visitor
	 */
	public static void dfs(mxAnalysisGraph aGraph, Object startVertex,
	 ...
	 * @param visitor
	 */
	private static void dfsRec(mxAnalysisGraph aGraph, Object cell,
			Object edge, Set<Object> seen, mxICellVisitor visitor) {
		if (cell != null) {
	 ...
	 * @param visitor
	 */
	public static void bfs(mxAnalysisGraph aGraph, Object startVertex,
			mxICellVisitor visitor) {
	 ...
	 * @param visitor
	 */
	private static void bfsRec(mxAnalysisGraph aGraph, Set<Object> queued,
			LinkedList<Object[]> queue, mxICellVisitor visitor) {
		if (queue.size() > 0) {
	 ...
	 * 
	 * <pre>
	 * mxTraversal.dijkstra(analysisGraph, startVertex, endVertex,
	 * 		new mxICellVisitor() {
	 * 			public boolean visit(Object vertex, Object edge) {
	 * 				// perform your processing on each cell here
	 * 				return false;
	 ...
	 *             graphs
	 */
	public static void dijkstra(mxAnalysisGraph aGraph, Object startVertex,
			Object endVertex, mxICellVisitor visitor)
			throws StructuralException {
		...
		while (vertexList.size() > 0) {
			...
			Object[] neighborVertices = aGraph.getOpposites(aGraph.getEdges(
					closestVertex, null, true, true, false, true),
			...
			for (int j = 0; j < neighborVertices.length; j++) {
				...
				if (vertexList.contains(currNeighbor)) {
					...
					Object[] neighborEdges = aGraph.getEdges(currNeighbor,
					...
					for (int k = 0; k < neighborEdges.length; k++) {
						...
						if (aGraph.getTerminal(currEdge, true).equals(
								closestVertex)
								|| aGraph.getTerminal(currEdge, false).equals(
										closestVertex)) {
					...
					currEdgeWeight = costFunction.getCost(new mxCellState(view,
							connectingEdge, null));
	...
	public static List<Map<Object, Object>> bellmanFord(mxAnalysisGraph aGraph,
		...
		for (int i = 0; i < vertexNum; i++) {
			for (int j = 0; j < edgeNum; j++) {
				...
				double dist = (Double) distanceMap.get(source)
						+ costFunction.getCost(new mxCellState(view, currEdge,
								null));
				...
				if (!mxGraphProperties.isDirected(aGraph.getProperties(),
					...
					dist = (Double) distanceMap.get(target)
							+ costFunction.getCost(new mxCellState(view,
									currEdge, null));
		...
		for (int i = 0; i < edgeNum; i++) {
			...
			double dist = (Double) distanceMap.get(source)
					+ costFunction
							.getCost(new mxCellState(view, currEdge, null));
			...
			for (int i = 0; i < vertices.length; i++) {
				for (int j = 0; j < vertices.length; j++) {
					if (dist[i][j] > dist[i][k] + dist[k][j]) {
						paths[i][j] = mxGraphStructure.getVertexWithValue(
								aGraph, k);
	 ...
	 * @return
	 */
	private static Double[][] initializeWeight(mxAnalysisGraph aGraph,
			Object[] nodes, Object[] edges, Map<Object, Integer> indexMap) {
		...
		for (Object currEdge : edges) {
			...
			weight[indexMap.get(source)][indexMap.get(target)] = costFunction
					.getCost(view.getState(currEdge));

			if (!isDirected) {
				weight[indexMap.get(target)][indexMap.get(source)] = costFunction
						.getCost(view.getState(currEdge));
			}
		}
	...
	public static Object[] getWFIPath(mxAnalysisGraph aGraph,
		...
		if (startVertex != targetVertex) {
			...
			while (startVertex != targetVertex) {
				result = getWFIPathRec(aGraph, paths, startVertex,
						targetVertex, currPath, cf, view);
	 ...
	 * @throws StructuralException
	 */
	private static ArrayList<Object> getWFIPathRec(mxAnalysisGraph aGraph,
			Object[][] paths, Object startVertex, Object targetVertex,
			ArrayList<Object> currPath, mxCostFunction cf, mxGraphView view)
			throws StructuralException {
		...
		if (parents[tIndex] != null) {
			currPath = getWFIPathRec(aGraph, paths, startVertex,
					parents[tIndex], currPath, cf, view);
		} else {
			if (mxGraphStructure
					.areConnected(aGraph, startVertex, targetVertex)
					|| startVertex == targetVertex) {
				currPath.add(targetVertex);
			} else {
				throw new StructuralException(
						"The two vertices aren't connected");
			}
		}
