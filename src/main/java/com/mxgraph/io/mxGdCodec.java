...
public class mxGdCodec {
				...
				case NUM_NODES: {
					if (!line.startsWith("#")) {
						...
						for (int i = 0; i < numVertices; i++) {
							...
							Object vertex = graph.insertVertex(parent, label,
									label, 0, 0, 10, 10);
				...
				case PARSING_NODES: {
					if (line.startsWith("# Edges")) {
						...
						if (items.length != 5) {
							...
							Object vertex = graph.insertVertex(parent, label,
									label, x - width / 2.0, y - height / 2.0,
									width, height);
	...
	public static String encode(mxGraph graph) {
		...
		Object parent = graph.getDefaultParent();
		Object[] vertices = mxGraphModel.getChildCells(graph.getModel(),
				parent, true, false);

		builder.append("# Number of Nodes (0-"
				+ String.valueOf(vertices.length - 1) + ")");
