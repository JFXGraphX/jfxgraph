				...
				if (connectPreview.isActive()
					...
					try {
						...
						if (!marker.hasValidState() && isCreateTarget()) {
							Object vertex = createTargetVertex(e,
									source.getCell());
							dropTarget = graph
									.getDropTarget(new Object[] { vertex }, e
											.getPoint(), graphComponent
							...
							if (vertex != null) {
								...
								if (dropTarget == null
									...
									mxCellState pstate = graph.getView()
											.getState(dropTarget);
								...
								graph.addCells(new Object[] { vertex },
										dropTarget);
							}
