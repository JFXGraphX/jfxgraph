...
public class mxSpline1D {
	...
	public double getValue(double x) {
		...
		return a[index] + b[index] * (x - len[index]) + c[index]
				* Math.pow(x - len[index], 2) + d[index]
	...
	public double getFastValue(double x) {
		...
		if (storageIndex > -1 && storageIndex < len.length - 1
				&& x > len[storageIndex] && x < len[storageIndex + 1]) {
		...
		return a[storageIndex] + b[storageIndex] * value + c[storageIndex]
				* (value * value) + d[storageIndex] * (value * value * value);
	}
	...
	public double getDx(double x) {
		...
		return b[index] + 2 * c[index] * (x - len[index]) + 3 * d[index]
				* Math.pow(x - len[index], 2);
	}
	...
	private void calculateCoefficients() {
		...
		for (int i = 0; i < N - 2; i++) {
			y[i] = 3 * ((pos1D[i + 2] - pos1D[i + 1]) / h[i + 1] - (pos1D[i + 1] - pos1D[i])
					/ h[i]);
