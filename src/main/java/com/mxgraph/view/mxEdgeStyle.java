...
public class mxEdgeStyle {
	...
	public static mxEdgeStyleFunction EntityRelation = new mxEdgeStyleFunction() {
		 ...
		 * 
		 * @see
		 * com.mxgraph.view.mxEdgeStyle.mxEdgeStyleFunction#apply(com.mxgraph
		 * .view.mxCellState, com.mxgraph.view.mxCellState,
		 * com.mxgraph.view.mxCellState, java.util.List, java.util.List)
		 */
		public void apply(mxCellState state, mxCellState source,
			...
			if (p0 != null) {
				...
				int constraint = mxUtils.getPortConstraints(source, state,
						true, mxConstants.DIRECTION_MASK_NONE);
			...
			if (pe != null) {
				...
				int constraint = mxUtils.getPortConstraints(target, state,
						false, mxConstants.DIRECTION_MASK_NONE);
	...
	public static mxEdgeStyleFunction SideToSide = new mxEdgeStyleFunction() {
		...
		public void apply(mxCellState state, mxCellState source,
			...
			if (source != null && target != null) {
				...
				if (result.size() == 1) {
					if (pt != null) {
						if (!target.contains(x, pt.getY())
								&& !source.contains(x, pt.getY())) {
	...
	public static mxEdgeStyleFunction TopToBottom = new mxEdgeStyleFunction() {
		...
		public void apply(mxCellState state, mxCellState source,
			...
			if (source != null && target != null) {
				...
				if (result.size() == 1) {
					if (pt != null) {
						if (!target.contains(pt.getX(), y)
								&& !source.contains(pt.getX(), y)) {
	...
	public static mxEdgeStyleFunction SegmentConnector = new mxEdgeStyleFunction() {
		 ...
		 */
		public void apply(mxCellState state, mxCellState source,
				mxCellState target, List<mxPoint> hints, List<mxPoint> result) {
			...
			if (pt == null && source != null) {
				pt = new mxPoint(state.view.getRoutingCenterX(source),
			...
			if (hints != null && hints.size() > 0) {
				...
				for (int i = 0; i < 2; i++) {
					boolean fixedVertAlign = currentPt != null
							&& currentPt.getX() == currentHint.getX();
					boolean fixedHozAlign = currentPt != null
							&& currentPt.getY() == currentHint.getY();
					boolean inHozChan = currentTerm != null
							&& (currentHint.getY() >= currentTerm.getY() && currentHint
									.getY() <= currentTerm.getY()
									+ currentTerm.getHeight());
					boolean inVertChan = currentTerm != null
							&& (currentHint.getX() >= currentTerm.getX() && currentHint
									.getX() <= currentTerm.getX()
									+ currentTerm.getWidth());

					hozChan = fixedHozAlign || (currentPt == null && inHozChan);
					vertChan = fixedVertAlign
							|| (currentPt == null && inVertChan);
					...
					if (vertChan || hozChan) {
						...
						if (i == 1) {
							...
							horizontal = hints.size() % 2 == 0 ? hozChan
									: vertChan;
						}
					...
					currentHint = state.view.transformControlPoint(state,
							hints.get(hintsLen - 1));
				}

				if (horizontal
						&& ((pts.get(0) != null && pts.get(0).getY() != hint
								.getY()) || (pts.get(0) == null
								&& source != null && (hint.getY() < source
								.getY() || hint.getY() > source.getY()
								+ source.getHeight())))) {
					result.add(new mxPoint(pt.getX(), hint.getY()));
				} else if (!horizontal
						&& ((pts.get(0) != null && pts.get(0).getX() != hint
								.getX()) || (pts.get(0) == null
								&& source != null && (hint.getX() < source
								.getX() || hint.getX() > source.getX()
								+ source.getWidth())))) {
				...
				for (int i = 0; i < hints.size(); i++) {
					...
					hint = state.view
							.transformControlPoint(state, hints.get(i));
			...
			if (horizontal
					&& ((pts.get(lastInx) != null && pts.get(lastInx).getY() != hint
							.getY()) || (pts.get(lastInx) == null
							&& target != null && (hint.getY() < target.getY() || hint
							.getY() > target.getY() + target.getHeight())))) {
				result.add(new mxPoint(pt.getX(), hint.getY()));
			} else if (!horizontal
					&& ((pts.get(lastInx) != null && pts.get(lastInx).getX() != hint
							.getX()) || (pts.get(lastInx) == null
							&& target != null && (hint.getX() < target.getX() || hint
			...
			if (pts.get(0) == null && source != null) {
				while (result.size() > 1
						&& source.contains(result.get(1).getX(), result.get(1)
								.getY())) {
					result.remove(1);
				}
			}
			...
			if (pts.get(lastInx) == null && target != null) {
				while (result.size() > 1
						&& target.contains(
								result.get(result.size() - 1).getX(), result
										.get(result.size() - 1).getY())) {
	...
	public static mxEdgeStyleFunction OrthConnector = new mxEdgeStyleFunction() {
		...
		public void apply(mxCellState state, mxCellState source,
			...
			boolean sourceEdge = source == null ? false : graph.getModel()
					.isEdge(source.cell);
			boolean targetEdge = target == null ? false : graph.getModel()
					.isEdge(target.cell);

			if ((points != null && points.size() > 0) || (sourceEdge)
					|| (targetEdge)) {
				mxEdgeStyle.SegmentConnector.apply(state, source, target,
						points, result);
				return;
			}
